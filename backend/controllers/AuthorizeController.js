...
class AuthorizeController {
    signup = async (req, res) => {
        ...
        try {
            ...
            await newUser.save();
            res.status(201).json({ success: true, message: 'Người dùng đã được tạo thành công' });

        } catch (err) {
            if (err.message === 'username_exists') {
                res.status(400).json({ success: false, message: 'Tên tài khoản đã tồn tại' });
            } else if (err.message === 'invalid_email') {
                res.status(400).json({ success: false, message: 'Email sai định dạng' });
            } else if (err.message === 'invalid_phone') {
                res.status(400).json({ success: false, message: 'Số điện thoại sai định dạng' });
            } else {
                console.error(err);
    ...
    login = async (req, res) => {
        ...
        try {
            ...
            if (err.message === 'user_not_found') {
                res.status(400).json({ success: false, message: 'Người dùng không tồn tại' });
