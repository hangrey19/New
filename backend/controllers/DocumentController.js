...
class DocumentController {
    upload = async (req, res) => {
        try {
            ...
            if (!userLink || typeof userLink !== 'string' || userLink.trim() === '') {
                return res.status(400).json({ success: false, message: 'Bạn phải nhập link tài liệu!' });
            }
            ...
            const attachedFiles = [userLink];

            // Nếu bạn muốn lưu thêm thuộc tính khác, ví dụ tên file hoặc extension
            // có thể parse từ link ở đây, ví dụ:
            // let fileAttributes = [{ fileName: userLink.split('/').pop() }];
            ...
            return res.status(200).json({ success: true, message: 'Tài liệu đã được lưu!' });
        } catch (err) {
            console.log(err);
