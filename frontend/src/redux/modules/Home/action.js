...
// action Fetch Classroom Info
export const fetchClassroomInfo = (classroomId) => {
  let accessToken = "";
  if (localStorage.getItem("User"))
    accessToken = JSON.parse(localStorage.getItem("User")).token;

  return (dispatch) => {
    dispatch(fetchClassroomInfoRequest());
    axios({
      ...
      .then((res) => {
        dispatch(fetchClassroomInfoSuccess(res.data));
      })
      .catch((err) => {
        dispatch(fetchClassroomInfoFailed(err));
      });
  };
};

const fetchClassroomInfoRequest = () => {
  return {
    type: actionTypes.FETCH_CLASSROOM_INFO_REQUEST,
  };
};

const fetchClassroomInfoSuccess = (data) => {
  return {
    type: actionTypes.FETCH_CLASSROOM_INFO_SUCCESS,
    payload: data,
  };
};

const fetchClassroomInfoFailed = (err) => {
  return {
    type: actionTypes.FETCH_CLASSROOM_INFO_FAILED,
    payload: err,
  };
};
