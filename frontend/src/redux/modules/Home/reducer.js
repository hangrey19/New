...
// Fetch Classroom Info Reducer
const stateClassroomInfo = {
  loading: false,
  data: null,
  err: null,
};

export const fetchClassroomInfoReducer = (
  state = stateClassroomInfo,
  { type, payload }
) => {
  switch (type) {
    case actionTypes.FETCH_CLASSROOM_INFO_REQUEST:
      state.loading = true;
      state.data = null;
      state.err = null;
      return { ...state };
    case actionTypes.FETCH_CLASSROOM_INFO_SUCCESS:
      state.loading = false;
      state.data = payload;
      state.err = null;
      return { ...state };
    case actionTypes.FETCH_CLASSROOM_INFO_FAILED:
      state.loading = false;
      state.data = null;
      state.err = payload;
      return { ...state };
...
export const joinClassroomReducer = (
  state = stateJoinClassroom,
  { type, payload }
) => {
  switch (type) {
...
export const leaveClassroomReducer = (
  state = stateLeaveClassroom,
  { type, payload }
) => {
  switch (type) {
