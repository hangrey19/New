...
function ListMember() {
  ...
  useEffect(() => {
    if (classroomId) {
      console.log("Fetching people for classroom:", classroomId);
      dispatch(actFetchPeopleList(classroomId));
    } else {
      console.log("No classroomId found in useParams");
    }
  ...
  console.log("ListMember state:", { data, loading, err, classroomId });
  ...
  const handleDelete = () => {
    dispatch(actDeleteStudent(classroomId, studentDelete._id));
    setRender(!render);
  };
